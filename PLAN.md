...
| [30](plans/iteration-30.md) | Counter-protocols: drafting a protocol gives every act-2 enemy a same-tier answer (silver stat trim / gold build-answer / prismatic rule-breaker), shown on the draft card before the pick — pure stat data on existing engine fields, no combat-engine changes | **specced 2026-08-06, not implemented** |
| [31](plans/iteration-31.md) | The Foundry: fuse permanent slotless base-stat points into a hull at any trade station (escalating prices — the late-run credit sink) + the act-2 final-boss trio measured against a protocols-era endgame fleet for the first time and buffed to a 25-55% band (M3 depends on 30) | **specced 2026-08-06, not implemented** |
